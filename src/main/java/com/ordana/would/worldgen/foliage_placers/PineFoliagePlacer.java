package com.ordana.would.worldgen.foliage_placers;
...
public class PineFoliagePlacer extends FoliagePlacer {
    public static final MapCodec<PineFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(foliagePlacerInstance ->
    ...
    protected void createFoliage(WorldGenLevel level, FoliageSetter blockSetter, RandomSource random, TreeConfiguration config, int maxFreeTreeHeight, FoliageAttachment attachment, int foliageHeight, int foliageRadius, int offset) {
        ...
        Direction extendedLeaves = values[attachment.radiusOffset()];
        BlockPos basePos = attachment.pos();
        BlockPos.MutableBlockPos pos = basePos.mutable();
        for (Direction dir : values) {
            if (dir == Direction.DOWN) continue;
            if (dir == extendedLeaves) tryPlaceLeaf(level, blockSetter, random, config, pos.relative(extendedLeaves, 2));
